...
export const onRequestPost = async ({ request, env }: FunctionContext) => {
  try {
    ...
    if (isClubVacancy && (!accountClub || !["owner", "captain", "admin"].includes(profile.role))) return apiError("Vincule seu próprio clube e entre como dono ou capitão para anunciar uma vaga.", 403);
