...
export const onRequestGet = async ({ request, env }: FunctionContext) => {
  try {
    ...
    const payload = rows.map((row) => ({ id: row.id, mode: row.mode, playedAt: row.played_at, homeClubId: row.home_club_id && byId.get(row.home_club_id) ? publicRouteId(byId.get(row.home_club_id)!) : row.home_ea_club_id, homeClubName: row.home_club_name, awayClubId: row.away_club_id && byId.get(row.away_club_id) ? publicRouteId(byId.get(row.away_club_id)!) : row.away_ea_club_id, awayClubName: row.away_club_name, homeScore: row.home_score, awayScore: row.away_score, competition: row.competition, sourceUrl: row.source_url, players: Array.isArray(row.players) ? row.players : [] }));
