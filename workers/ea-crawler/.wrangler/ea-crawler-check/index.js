...
// index.mjs
var PARSER_VERSION = "cloudflare-browser-public-page-v1";
var USER_AGENT = "ProClubsAmericaCrawler/1.0 (+https://proclubsamerica.com)";
var sleep = /* @__PURE__ */ __name((milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds)), "sleep");
var safeText = /* @__PURE__ */ __name((value, fallback = "") => String(value ?? fallback).trim(), "safeText");
var safeNumber = /* @__PURE__ */ __name((value, fallback = 0) => Number.isFinite(Number(value)) ? Number(value) : fallback, "safeNumber");
...
function playedAt(value) {
  ...
  if (Number.isFinite(numeric) && numeric > 0) return new Date(numeric < 1e10 ? numeric * 1e3 : numeric).toISOString();
  const parsed = new Date(String(value || ""));
  return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString();
}
__name(playedAt, "playedAt");
function players(payload, clubIds) {
  ...
  return clubIds.flatMap((clubId) => {
    ...
    return (Array.isArray(collection) ? collection : Object.values(collection)).flatMap((player) => {
      ...
      return name ? [{ playerId: safeText(player.playerId || player.nucleusId || name), playerName: name, position: safeText(player.pos || player.position || player.favoritePosition), goals: safeNumber(player.goals), assists: safeNumber(player.assists), rating: player.rating == null ? void 0 : safeNumber(player.rating), shots: player.shots == null ? void 0 : safeNumber(player.shots), passesMade: player.passesMade == null ? void 0 : safeNumber(player.passesMade), passAttempts: player.passAttempts == null ? void 0 : safeNumber(player.passAttempts), tacklesMade: player.tacklesMade == null ? void 0 : safeNumber(player.tacklesMade), tackleAttempts: player.tackleAttempts == null ? void 0 : safeNumber(player.tackleAttempts), redCards: player.redCards == null ? void 0 : safeNumber(player.redCards), saves: player.saves == null ? void 0 : safeNumber(player.saves), cleanSheet: Boolean(player.cleanSheet || player.cleanSheets) }] : [];
    });
  });
}
__name(players, "players");
...
__name(robotsAllows, "robotsAllows");
async function clickMode(page, labels) {
  return page.evaluate((wanted) => {
    const elements = [];
    const visit = /* @__PURE__ */ __name((root) => {
...
async function crawl(env, item) {
  ...
  const browser = await puppeteer_cloudflare_default.launch(env.BROWSER, { keep_alive: 12e4 });
  const page = await browser.newPage();
  const observed = [];
  const responseTasks = [];
  ...
  try {
    ...
    const response = await page.goto(sourceUrl, { waitUntil: "domcontentloaded", timeout: 45e3 });
    ...
    return { status: observed.length ? "succeeded" : "failed", responseCount: observed.length, error: observed.length ? void 0 : "PUBLIC_PAGE_DATA_NOT_OBSERVED", matches };
  } catch (error) {
...
__name(run, "run");
var index_default = {
  async scheduled(_controller, env, context2) {
    context2.waitUntil(run(env).then((result) => console.log(JSON.stringify(result))).catch((error) => console.error(error)));
  },
  async fetch(request, env) {
    ...
    try {
      ...
      return Response.json({ error: error instanceof Error ? error.message : "CRAWLER_FAILED" }, { status: 500 });
    }
  }
};
export {
