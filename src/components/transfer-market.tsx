...
import { applyToTransferPost, getTransferApplications, publishTransferPost, watchTransferPosts, type MarketApplicationRecord, type TransferPostRecord } from "@/lib/community-service";
...
export function TransferMarket({ initialType = "club_vacancy" }: { initialType?: TransferPost["type"] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!user) { setNotice("Entre na sua conta para publicar no mercado."); return; } const formElement = event.currentTarget; const form = new FormData(formElement); const minimumOvr = Number(form.get("minimumOvr") || 0) || undefined; setBusy(true); setNotice(""); try { const created = await publishTransferPost({ type, title: String(form.get("title")), owner: String(form.get("owner")), position: String(form.get("position")), minimumOvr, platform: String(form.get("platform")), availability: String(form.get("availability")), contact: String(form.get("contact")) }); setPosts((current) => [created, ...current.filter((post) => post.id !== created.id)]); formElement.reset(); setNotice("Anúncio publicado para toda a comunidade."); } catch { setNotice("Não foi possível publicar o anúncio."); } finally { setBusy(false); } }
