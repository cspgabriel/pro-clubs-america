...
export function TransferMarket({ initialType = "club_vacancy" }: { initialType?: TransferPost["type"] }) {
  ...
  // So pedimos push depois de uma acao que gera resposta de outra pessoa.
  ...
  async function apply(post: TransferPost) {
    ...
    try { await applyToTransferPost(post.id); setPosts((current) => current.map((item) => item.id === post.id ? { ...item, hasApplied: true, applicationCount: (item.applicationCount || 0) + 1 } : item)); setNotice(`Candidatura enviada para ${post.owner}.`); setAwaitingReply(true); }
