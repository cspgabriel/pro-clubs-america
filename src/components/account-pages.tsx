...
export function AccountPage() {
  ...
  return <main className="app-shell"><PlatformHeader /><section className="account-hero professional"><span className="account-avatar">{(user?.name ?? "J").slice(0, 1).toUpperCase()}</span><div><small>MINHA CONTA</small><h1>{user?.name ?? "Você ainda não entrou"}</h1><p>{user?.email ?? "Entre para aceitar desafios e representar seu clube."}</p></div></section><div className="account-grid professional">{user ? <>
    <article className="account-profile-card"><header><CheckCircle2 /><span>CONTA VERIFICADA</span></header><div className="account-identity"><span>{user.name.slice(0, 1).toUpperCase()}</span><div><h2 className="profile-country-title">{user.name} <CountryFlag country={profile?.country} /></h2><p><Mail /> {user.email}</p></div></div><dl><div><dt>Perfil</dt><dd>{profile?.role === "owner" ? "Dono" : profile?.role === "captain" ? "Capitão" : profile?.role === "player" ? "Jogador" : "Visitante"}</dd></div><div><dt>Plano</dt><dd>{profile?.plan ?? "free"}</dd></div>{profile?.bonusAccessUntil && <div><dt>Bônus ativo</dt><dd>{new Date(profile.bonusAccessUntil).toLocaleDateString("pt-BR")}</dd></div>}</dl><div className="account-card-actions"><Link href="/onboarding"><Settings2 /> País e idioma <ArrowRight /></Link>{profile?.plan && profile.plan !== "free" ? <BillingPortalButton /> : <Link href="/planos"><CalendarCheck /> Conhecer planos <ArrowRight /></Link>}</div></article>
    <article className="account-club-card"><header><Shield /><span>MEU CLUBE</span></header><h2 className="profile-country-title">{profile?.clubName ?? profile?.pendingClubName ?? "Vincule um time EA"}<CountryFlag country={profile?.country} /></h2><p>{profile?.clubId ? `Você faz parte deste clube como ${profile.role === "owner" ? "dono" : profile.role === "captain" ? "capitão" : "jogador"}.` : profile?.pendingClubId ? "Solicitação enviada. O vínculo será liberado após a validação da comunidade." : "Vincule o clube para aceitar desafios e convidar integrantes do elenco."}</p><div className="account-card-actions">{profile?.clubId && <Link href="/conta/time"><Users /> Gerenciar time <ArrowRight /></Link>}<Link href={profile?.clubId ? `/club/${profile.clubId}` : profile?.pendingClubId ? "/rankings/times" : "/cadastro"}>{profile?.clubId ? "Abrir página do clube" : profile?.pendingClubId ? "Acompanhar validação" : "Cadastrar meu time"} <ArrowRight /></Link></div></article>
