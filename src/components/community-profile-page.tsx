...
export function CommunityProfilePage() {
  ...
  return <main className="app-shell"><PlatformHeader />{profile ? <>
    <section className="account-hero professional"><span className="account-avatar">{profile.name.slice(0, 1).toUpperCase()}</span><div><small>PERFIL DA COMUNIDADE</small><h1 className="profile-country-title">{profile.name} <CountryFlag country={profile.country} /></h1><p>{roleLabel(profile.role)}{profile.club ? ` · ${profile.club.name}` : ""}</p></div></section>
