...
export function RegistrationPage({ indexedClubs }: { indexedClubs: IndexedClubOption[] }) {
  ...
  // Caminho primario e achar o clube no catalogo indexado (557 clubes): dai nome,
  // plataforma e URL saem prontos. O preenchimento manual vira excecao.
    ...
    <div className="registration-layout">
      {submitted ? <section className="registration-success"><CheckCircle2 /><small>TIME VINCULADO</small><h2>{submitted.clubName}</h2><p>Você agora representa o clube EA ID <b>{submitted.clubId}</b> como dono. O vínculo é imediato; resultados e estatísticas continuam validados pela fonte oficial.</p><div><Clock3 /><span>STATUS ATUAL<strong>{submitted.ea?.synced ? `Vínculo ativo · ${submitted.ea.matches} partidas da EA já carregadas` : "Vínculo ativo · buscando os dados da EA, aparecem em instantes"}</strong></span></div><Link href="/conta/time">Gerenciar meu time</Link></section> : <form className="registration-form" onSubmit={submit}>
        ...
        {user?.name && user?.email
          ? <><input type="hidden" name="responsibleName" value={user.name} /><input type="hidden" name="email" value={user.email} />
              <p className="registration-identity"><ShieldCheck /> Vinculando como <b>{user.name}</b> · {user.email}</p></>
        ...
        <div className="registration-pair">{(manualMode || !selectedClub) && <label>Nome do time<input required name="clubName" value={clubName} onChange={(event) => setClubName(event.target.value)} /></label>}
          {selectedClub && !manualMode && <input type="hidden" name="clubName" value={clubName} />}
          <label>País do clube<select required name="country" defaultValue="brasil">{countries.map((country) => <option value={country.slug} key={country.code}>{country.flag} {country.name.pt}</option>)}</select></label></div>
        {selectedClub && !manualMode
          ? <><input type="hidden" name="eaUrl" value={eaUrl} />
              <p className="registration-autofilled"><CheckCircle2 /> Link da EA preenchido automaticamente pelo catálogo. <button type="button" onClick={() => setManualMode(true)}>Editar manualmente</button></p></>
