...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    func initWebView() {
        ...
        if(pullToRefresh){
            ...
            ProClubsAmerica.webView.scrollView.addSubview(refreshControl)
            ProClubsAmerica.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = ProClubsAmerica.webView.observe(\.themeColor) { [unowned self] webView, _ in
                let backgroundColor = ProClubsAmerica.webView.underPageBackgroundColor;
                let themeColor = ProClubsAmerica.webView.themeColor;
    ...
    @objc func refreshWebView(_ sender: UIRefreshControl) {
        ProClubsAmerica.webView?.reload()
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                ProClubsAmerica.webView.isLoading &&
